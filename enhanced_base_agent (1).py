"""
Enhanced Production-Ready Base Agent v3.1
==========================================
...
- Fixed metric collection methods
...
class CircuitBreaker:
    """Enhanced circuit breaker with half-open state and better tracking"""
    ...
    def __init__(
        ...
        # Track recent failures for better diagnostics
...
class BaseAgent:
    ...
    This is the foundation for all specialized agents in the system.
    """
    ...
    def _setup_logging(self):
        ...
        self.logger = logging.getLogger(f"{self.config.agent_type}.{self.config.name}")
    ...
    async def start(self) -> bool:
        ...
        try:
            ...
            if self.config.nats_url and NATS_AVAILABLE:
                connection_tasks.append(('nats', self._connect_nats()))
            
            if self.config.postgres_url and POSTGRES_AVAILABLE:
                connection_tasks.append(('postgres', self._connect_postgres()))
            
            if self.config.redis_url and REDIS_AVAILABLE:
                connection_tasks.append(('redis', self._connect_redis()))
            ...
            if connection_tasks:
                ...
                for (name, _), result in zip(connection_tasks, results):
                    if isinstance(result, Exception):
                        self.logger.error(f"{name.upper()} connection failed: {result}")
                    elif result:
                        self.logger.info(f"{name.upper()} connected successfully")
                
                # Check for critical failures (NATS is critical if configured)
    ...
    async def _connect_redis(self) -> bool:
        ...
        if not REDIS_AVAILABLE:
            self.logger.warning("redis library not installed")
            return False
    ...
    async def _setup_subscriptions(self):
        """Setup NATS subscriptions - override to add more"""
        if not self.nc:
            return
    ...
    # Redis Methods with Circuit Breaker
    async def _redis_get(self, key: str, timeout: float = 5.0) -> Optional[str]:
        ...
        try:
            return await self.redis_circuit_breaker.call(
                lambda: asyncio.wait_for(self.redis.get(key), timeout=timeout)
            )
        except Exception as e:
    ...
    async def _redis_set(self, key: str, value: str, ex: Optional[int] = None, timeout: float = 5.0) -> bool:
        ...
        try:
            await self.redis_circuit_breaker.call(
                lambda: asyncio.wait_for(self.redis.set(key, value, ex=ex), timeout=timeout)
            )
            return True
        except Exception as e:
            self.metrics.redis_errors += 1
        ...
        try:
            ...
            if self.background_tasks:
                try:
                    ...
                    self.logger.warning("Background tasks shutdown timeout")
...
if __name__ == "__main__":
    ...
    config = AgentConfig(
        agent_id=os.getenv("AGENT_ID", "base-001"),
        name=os.getenv("AGENT_NAME", "base_agent"),
        agent_type=os.getenv("AGENT_TYPE", "base"),
